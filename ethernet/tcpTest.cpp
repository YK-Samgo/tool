...
#define TARGET_IP "192.168.143.10"

int main(int argC, char* argv[])
{ 
  int test_size = SIZE_128M * 8;
  ...
  while(1) {
    rcvdData[0] = "\0";
      ...
      /* 接收server端返回的字符串*/
    bufferRest = std::min(655350, rb.canWrite());
    ...
    while (rb.canWrite() < len){usleep(100);}
   ...
   //}else if(diskWrite == 0){
    ...
    while (1){
      if (bytesRead = rb.read(temp, 1048576)){
        FILE *fp;
        fp = fopen("/mnt/data/test1.data","a");
        ...
        usleep(100);
        if (nevent % 10000 == 0){
          printf("Band Width: %1.2f Gbps\n", double(bytesGet) / 1024 / 1024 / 128 /difftime(time(0), time0));
          time0 = time(0);
          bytesGet = 0;
        }
      }else{
