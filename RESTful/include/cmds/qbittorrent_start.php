...
function qbittorrent_start()
{
    ...
    $commamd = 'sudo service qbittorrent start';
    exec($commamd);
...
// ----------------------------------------------------------------------------
// 以下为内置的测试代码，可以删除或修改：
function qbittorrent_start__tunit__()
{
    if ( ! defined("APPID") )
    {
        $title = "正在执行接口测试 cmd: qbittorrent_start ...";

        require_once(dirname(__FILE__) . "/../iunit_common.php");

        $cmdObject = (object)["cmd"=>"qbittorrent_start"
